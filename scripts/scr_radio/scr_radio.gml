/// @feather ignore all
function Radio(text, styles = {}, x = LAYOUT_AUTO, y = LAYOUT_AUTO) : Component(x, y, styles) constructor {
	#region Properties
	self.value = text;
	self.selected = false;
	self.group = noone;
	...
	self.setX = function(x) {
		self.x = x;
		self.text.setX(self.getX());
		return self;
	}
	
	self.setY = function(y) {
		self.y = y;
		self.text.setY(self.getY());
		return self;
	}
	...
	self.setSelected = function(selected) {
		if (group != noone) {
			...
			group.events.onChange(self.value);
		}
	...
	self.draw_body = function() {	
		...
		// Radio background
		draw_set_color(self.getSelected() ? self.styles.fill : self.styles.backgroundColor);
		draw_set_alpha(self.getSelected() ? self.styles.fillOpacity : self.styles.opacity);
